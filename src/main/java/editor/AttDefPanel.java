...
public class AttDefPanel extends JPanel implements MouseListener {
	...
	public void paintComponent(Graphics g) {
		...
		if (selected == -1) {
			...
			if (Formations.getDef(of, squad, altBox.getSelectedIndex(),
					selected) == 1) {
				g2.setPaint(Color.gray);
				g2.draw(new Ellipse2D.Double(21, 21, 14, 14));
				g2.setPaint(Color.blue);
				g2.fill(new Ellipse2D.Double(21, 63, 14, 14));
			} else if (Formations.getDef(of, squad, altBox.getSelectedIndex(),
					selected) == 0) {
				g2.setPaint(Color.blue);
	...
	public void mousePressed(MouseEvent e) {
		...
		int count = 0;
		boolean boo = true;
		byte def = Formations.getDef(of, squad, altBox.getSelectedIndex(),
				selected);
		if (new Ellipse2D.Double(42, 42, 14, 14).contains(e.getX(), e.getY())) {
			Formations.setAtk(of, squad, altBox.getSelectedIndex(), selected,
					-1);
			Formations
					.setDef(of, squad, altBox.getSelectedIndex(), selected, 2);
		} else if (new Ellipse2D.Double(21, 21, 14, 14).contains(e.getX(), e
				.getY())) {
			if (def == 0) {
				Formations.setDef(of, squad, altBox.getSelectedIndex(),
						selected, 1);
			} else {
				Formations.setDef(of, squad, altBox.getSelectedIndex(),
						selected, 0);
			...
			for (int i = 0; boo & i < 8; i++) {
				if (attSqu[i].contains(e.getX(), e.getY())) {
					...
					if (Formations.getAtk(of, squad, altBox.getSelectedIndex(),
						...
						Formations.setAtk(of, squad, altBox.getSelectedIndex(),
								selected, i);
					} else if (count < 2) {
						if (selected != 0) {
							Formations.setAtk(of, squad, altBox
									.getSelectedIndex(), selected, i);
							// of.data[a + (altBox.getSelectedIndex() * 171)] =
							// 1;
						} else if (i == 0 || i == 4) {
							Formations.setAtk(of, squad, altBox
									.getSelectedIndex(), selected, i);
